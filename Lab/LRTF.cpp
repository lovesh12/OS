...
int getNextLRTFProcess(vector<process> &jobs, int currentTick)
{
    int n = jobs.size();

    int maxTimeRem = INT_MIN;
    int maxLongestJob = -1;

    for (int i = 0; i < n; ++i)
    {
        if (jobs[i].arrival <= currentTick && jobs[i].remTime > maxTimeRem && jobs[i].remTime > 0)
        {
            maxTimeRem = jobs[i].remTime;
            maxLongestJob = i;
        }
    }

    return maxLongestJob;
}
...
int main()
{
    ...
    {
        int minShortestJobIdx = getNextLRTFProcess(processes, currentTick);
