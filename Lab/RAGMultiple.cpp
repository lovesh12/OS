...
int main()
{
    ...
    cout << "Enter no. of resource instances allocated to each processes 1 to " << nP << " (0 for none): ";
    for (int i = 0; i < nP; i++)
    {
        int nI;
        cin >> nI;
        ...
        for (int j = 0; j < nI; j++)
        {
            ...
            {
                adjMatrix[nP + allocatedResource - 1][i]++;
            }
        }
    }

    cout << "Enter no. of resource instances requested by each processes 1 to " << nP << " (0 for none): ";
    for (int i = 0; i < nP; i++)
    {
        int nI;
        cin >> nI;
        cout << "Enter resource IDs requested by process " << i + 1 << ": ";
        for (int j = 0; j < nI; j++)
        {
            ...
            {
                adjMatrix[i][nP + requestedResource - 1]++;
            }
        }
    }
